...
function noData() {
    ...
    if (type === 'L') {
        pClass = 'localstorage-i';
        promptText = 'local';
    }
    else {
        pClass = 'sessionstorage-i';
        promptText = 'session';
    }
    return '<div class="infooo ' + pClass + '">No ' + promptText + ' storage data found</div>';
}
...
executeScript({ what: 'get', type: type }, function(response) {
    ...
    if (storage === undefined) {
        ...
        for (var i in storage) {
            ...
            str += key + '" data-key="' + key + '"readonly></td>';
            str += '<td class="td-value"><input type="text" value="';
            str += value + '"readonly></td>';
...
$('#download').click(function(e) {
   ...
   function(tabs){
    ...
    if (error) {
        alert('Error! There could be to much data to sync!');$('#errorr').show();
    }
...
$('#import').click(function(e) {
    e.preventDefault();
loading(true);
    var json = chrome.tabs.query({'active': true, 'windowId': chrome.windows.WINDOW_ID_CURRENT},
   function(tabs){
		var host = tabs[0];
		var url = new URL(host.url);
      var domain = url.hostname + ' (' + type + ')';
		 chrome.storage.sync.get(domain, function (reee) {
			 var error = chrome.runtime.lastError;
    if (error) {
        alert('Error! There could be to much data to sync!');$('#errorr').show();
    } 
			 ...
			 console.log(foo);
			 loading(false);
		executeScript({ type: type, what: 'import', json: foo }, function() {
            location.reload();
        });
});
   }
